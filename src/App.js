...
function App() {
    ...
    const handleNodeSelection = (event) => {
        ...
        if (selectedNodeId) {
            ...
            addPredecessors(selectedNodeId);
            addSuccessors(selectedNodeId);
    ...
    return (
        <div className="App">
            <header className="App-header">
                ...
                <select onChange={handleNodeSelection} value={selectedNode}>
                    <option value="">Select a resource</option>
                    {graph && graph.nodes().map(node => (
                        <option key={node} value={node}>{graph.node(node).label}</option>
                    ))}
                </select>
